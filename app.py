...
def normalize(text):
    """
    Zamienia polskie znaki na łacińskie i usuwa znaki interpunkcyjne; całość zamienia na małe litery.
    """
    text = text.lower()
    text = unidecode.unidecode(text)
    text = re.sub(r'[^a-z0-9 ]', ' ', text)  # usuwa znaki specjalne, zostawia spacje
    text = re.sub(r'\s+', ' ', text)         # pojedyncze odstępy
    return text.strip()
...
def search_answer(question, faq_base):
    # Szukaj najlepszej odpowiedzi: wybierz odpowiedź z największą liczbą wspólnych słów
